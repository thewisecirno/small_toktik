...
func UserRelationFollowList(c *gin.Context) {
	...
	log.Println("follow list>>>>>>", UserFollowList)
...
func UserRelationFollowerList(c *gin.Context) {
	...
	if err != nil {
		log.Println(err)
		responseUserRelationFailed(c, "userId parse error")
		return
	}
	followerList := models.GetUserFollowerList(userIdInt)
	log.Println("Follower list>>>>>>", followerList)
	c.JSON(http.StatusOK, gin.H{
		"status_code": 0,
		"status_msg":  "get user follower list successfully",
